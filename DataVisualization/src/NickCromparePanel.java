...
public class NickCromparePanel extends ApplicationFrame{
    ...
    public NickCromparePanel(String variable, String[] teams, JPanel superPanel){
        ...
        //chartPanel.setPreferredSize(new java.awt.Dimension( 3600 , 2000 ) );
        ...
        for (int i = (arrayLmao.length - 1); i >= 0; i--) {
            for (int j = 1; j <= i; j++) {
                try{if (parse(arrayLmao[j - 1][column]) < parse(arrayLmao[j][column])) {
                    ...
                    System.out.println(arrayLmao[j-1][column] + ":" + arrayLmao[j][column]);
                    //e.printStackTrace();
                    //System.err.print(i+":"+j+" "+arrayLmao.length);}
                }
            }
        }
    }
    ...
    private CategoryDataset createDataset(){
        ...
        for(String[] array:arrayLmao) {
            ...
            try {
                ...
                if(count<30)
                    dataset.addValue((double)Math.round(parse(array[column])*10)/10, "", getTeamNumber(array) + "-" + count);
                count++;
            } catch (Exception e) {
            }
            //}
    ...
    private CategoryDataset getVariableMaxOfTeamDataSet(int column){
        ...
        for(String[] array:arrayLmao){
            if(count<30){
                dataset.addValue(getVariableMaxofTeam(getTeamNumber(array), column), "Max", getTeamNumber(array) + "-" + count);
            }
            count++;
        }
        return dataset;
